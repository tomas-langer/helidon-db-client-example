...
```
docker build -t helidon-examples-dbclient-jdbc .
```

## Start the application with Docker

```
docker run --rm -p 8080:8080 helidon-examples-dbclient-jdbc:latest
```
...
kubectl get service helidon-examples-dbclient-jdbc  # Get service info
```
...
```
docker build -t helidon-examples-dbclient-jdbc-native -f Dockerfile.native .
```

Start the application:

```
docker run --rm -p 8080:8080 helidon-examples-dbclient-jdbc-native:latest
```