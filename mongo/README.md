...
```
java -jar target/helidon-examples-dbclient-mongo.jar
```
...
```
docker build -t helidon-examples-dbclient-mongo .
```

## Start the application with Docker

```
docker run --rm -p 8080:8080 helidon-examples-dbclient-mongo:latest
```
...
kubectl get service helidon-examples-dbclient-mongo  # Get service info
```
...
```
./target/helidon-examples-dbclient-mongo
```
...
```
docker build -t helidon-examples-dbclient-mongo-native -f Dockerfile.native .
```

Start the application:

```
docker run --rm -p 8080:8080 helidon-examples-dbclient-mongo-native:latest
```